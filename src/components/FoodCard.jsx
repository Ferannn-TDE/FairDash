...
const FoodCard = ({ item }) => {
  ...
  const isDesktop = useMediaQuery('(min-width: 78.125rem)');
  ...
  const handleSelectSize = (itemWithSize) => {
    addToCart(itemWithSize);
    toast.success(`${itemWithSize.name} (${itemWithSize.sizeName}) added to cart! 🎉`);
      ...
      <div className="bg-bg-card rounded-xl md:rounded-2xl overflow-hidden border border-white/5 transition-all duration-300 ease-out hover:scale-[1.02] hover:shadow-glow hover:border-neon-pink/30 flex flex-col h-full animate-fadeIn">
        ...
        <div className="p-3 md:p-5 flex flex-col flex-1">
          ...
          >
            ...
            {item.sizes ? 'Choose Size' : 'Add to Cart'}
          </button>
        </div>
      </div>
