      ...
      <div className="flex items-center gap-5">
        ...
        {isSignedIn ? (
          <div className="flex items-center gap-3">
            ...
            <button
              className="px-4 py-2.5 bg-transparent border-2 border-white/20 text-text-gray font-semibold uppercase cursor-pointer transition-all duration-300 ease-out rounded-lg text-sm tracking-wide hover:border-neon-pink hover:text-neon-pink"
              onClick={() => signOut(() => navigate("/"))}
            >
              <ArrowRightOnRectangleIcon className="w-5 h-5 lg:hidden" />
              <span className="hidden lg:inline">Sign Out</span>
            </button>
          </div>
        ) : (
          <SignInButton mode="modal" forceRedirectUrl="/home">
            <button className="lg:block hidden px-[30px] py-3 bg-transparent border-2 border-neon-pink text-neon-pink font-semibold uppercase cursor-pointer transition-all duration-300 ease-out rounded-lg text-sm tracking-wide shadow-glow hover:bg-neon-pink hover:text-white hover:shadow-glow-intense">
              Sign In
            </button>
          </SignInButton>
        )}
