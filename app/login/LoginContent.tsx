...
// Compact heights on mobile, normal on desktop
const clerkElements = {
  rootBox: "w-full",
  ...
  socialButtonsBlockButton:
    "relative bg-white/[0.04] border border-white/[0.08] text-white hover:bg-white/[0.08] hover:border-white/[0.12] transition-all duration-200 rounded-xl h-10 font-inter mt-0",
  socialButtonsBlockButtonText: "font-inter text-sm text-gray-200 font-medium",
  socialButtonsProviderIcon: "w-4 h-4",
  dividerLine: "bg-white/[0.06]",
  dividerText:
    "text-gray-600 font-inter text-xs uppercase tracking-wider bg-[#111111] px-3",
  formFieldLabel:
    "text-[10px] font-inter text-gray-400 uppercase tracking-wider mb-1",
  formFieldInput:
    "w-full h-10 px-3 bg-white/[0.03] border border-white/[0.08] text-white text-sm font-inter rounded-xl placeholder:text-gray-600 focus:border-[#FF0077]/40 focus:ring-1 focus:ring-[#FF0077]/20 focus:bg-white/[0.04] transition-all duration-200 outline-none",
  formFieldInputShowPasswordButton: "text-gray-500 hover:text-gray-300",
  formButtonPrimary:
    "w-full h-10 bg-[#FF0077] hover:bg-[#FF0077]/90 active:scale-[0.98] text-white font-inter font-semibold text-sm uppercase tracking-wider rounded-xl transition-all duration-200 shadow-[0_4px_20px_rgba(255,0,119,0.25)] hover:shadow-[0_4px_30px_rgba(255,0,119,0.35)]",
  formFieldAction:
    "text-[#FF0077] hover:text-[#FF0077]/80 font-inter text-xs transition-colors",
  ...
  footerPages: "hidden",
  alert:
    "bg-red-500/10 border border-red-500/20 text-red-400 rounded-xl text-sm p-3",
};
...
export default function LoginContent() {
  ...
  const initialRole =
    rawRole && VALID_ROLES.includes(rawRole) ? rawRole : "customer";
  ...
  return (
    // h-screen + overflow-hidden → locks the page to exactly one viewport height on mobile
    <div className="h-screen overflow-hidden flex w-full">
      ...
      <div className="hidden lg:flex lg:w-[48%] relative overflow-hidden bg-[#0a0a0a]">
        ...
        <div className="relative z-10 flex flex-col h-full p-10 xl:p-14">
          <a href="/" className="group flex items-center w-fit">
            <span className="font-bebas text-2xl tracking-widest text-white transition-colors duration-200 group-hover:text-[#FF0077] leading-none">
              FAIR
            </span>
            <span className="font-bebas text-2xl tracking-widest text-[#FF0077] transition-colors duration-200 group-hover:text-white leading-none">
              SYNQ
            </span>
          </a>
      ...
      {/* ── RIGHT PANEL ─────────────────────────────────────────────────── */}
      <div className="flex-1 w-full h-screen overflow-hidden flex flex-col bg-[#111111] lg:rounded-l-3xl lg:border-l lg:border-white/[0.06] lg:shadow-[-20px_0_60px_rgba(0,0,0,0.3)] relative z-10">
        {/* Top accent line */}
        <div className="h-px w-full shrink-0 bg-gradient-to-r from-transparent via-[#FF0077]/30 to-transparent" />

        {/* ── Mobile header: logo + close on one row ── */}
        <div className="lg:hidden flex items-center justify-between w-full px-5 pt-3 pb-0 shrink-0">
          <a href="/" className="flex items-center">
            <span className="font-bebas text-xl tracking-widest text-white leading-none">
              FAIR
            </span>
            <span className="font-bebas text-xl tracking-widest text-[#FF0077] leading-none">
              SYNQ
            </span>
          </a>
          <a
            href="/"
            aria-label="Close"
            className="w-8 h-8 rounded-xl bg-white/[0.04] border border-white/[0.06] flex items-center justify-center hover:bg-white/[0.08] hover:border-white/[0.1] transition-all duration-200 group"
          >
            <X className="w-4 h-4 text-gray-500 group-hover:text-white transition-colors" />
          </a>
        </div>

        {/* Desktop close button (no logo needed — left panel has it) */}
        <div className="hidden lg:flex justify-end px-12 pt-6 shrink-0">
          <a
            href="/"
            aria-label="Close"
        ...
        {/* Mobile: role pills */}
        <div className="lg:hidden w-full px-5 pt-3 pb-1 shrink-0">
          <p className="text-[10px] text-gray-500 font-inter uppercase tracking-wider mb-2">
            I am a...
          </p>
          <div
            className="flex gap-1.5 overflow-x-auto w-full pb-1"
          ...
          >
            {ROLES.map((role) => (
              <button
                key={role.id}
                onClick={() => setSelectedRole(role.id)}
                className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-xs font-inter font-medium transition-all duration-200 cursor-pointer"
                style={{
                  background:
        ...
        {/* Auth form — flex-1 so it fills remaining height; centered */}
        <div className="flex-1 flex items-center justify-center px-5 sm:px-12 lg:px-16 py-2 overflow-hidden">
          <div className="w-full max-w-sm">
            ...
            <div className="flex items-center gap-2 mb-7">
              ...
              <div>
                <p className="text-[10px] text-gray-500 font-inter uppercase tracking-wider">
                  Signing in as
                </p>
                <p className="text-sm text-white font-inter font-medium leading-tight">
                  {activeRole.title}
                </p>
              </div>
            </div>
        ...
        <div className="hidden sm:flex px-6 py-4 shrink-0 items-center justify-between">
          ...
          <div className="flex items-center gap-4">
            <a
              href="/refund-policy"
              className="text-[10px] text-gray-700 hover:text-gray-400 font-inter transition-colors"
            >
              Privacy
            </a>
            <a
